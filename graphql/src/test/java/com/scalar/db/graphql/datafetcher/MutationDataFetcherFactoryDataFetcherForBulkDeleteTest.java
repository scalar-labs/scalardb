...
public class MutationDataFetcherFactoryDataFetcherForBulkDeleteTest extends DataFetcherTestBase {
  ...
  public void setUp() {
    ...
    dataFetcher = dataFetcherFactory::dataFetcherForBulkDelete;

    // table1_bulkDelete(put: [
    //   {
    //     key: { c1: 1, c2: "A" }
    //   }, {
    //     key: { c1: 1, c2: "B" }
    //   }
    // ])
    List<Map<String, Object>> deleteInput =
        ImmutableList.of(
            ImmutableMap.of("key", ImmutableMap.of(COL1, 1, COL2, "A")),
            ImmutableMap.of("key", ImmutableMap.of(COL1, 1, COL2, "B")));
    when(dataFetchingEnvironment.getArgument("delete")).thenReturn(deleteInput);
  }

  @Test
  public void get_WhenOperationSucceeds_ShouldReturnTrue() throws Exception {
    // Arrange
    doNothing().when(dataFetcherFactory).performDelete(eq(dataFetchingEnvironment), anyList());
  ...
  public void get_WhenOperationFails_ShouldReturnFalseWithErrors() throws Exception {
    ...
    doThrow(exception)
        ...
        .performDelete(eq(dataFetchingEnvironment), anyList());
