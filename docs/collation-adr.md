...
user-facing description of the feature lives in [collation.md](collation.md); the per-backend
research backing these decisions lives in
...
can therefore contradict what the backend's own scan returns.

### Decision

Introduce a single global setting, `scalar.db.collation` (`Collation.BINARY` or
...
- **Per-column or per-table collation.** Matches SQL engines' granularity, but multiplies
...
- A single `CollationComparator` (in `com.scalar.db.io`), built once from the config, is
  the sole comparison authority for every in-memory text comparison site.
...
determines how closely `ICU` mode aligns with a given backend, and any bump silently moves
that alignment.
...
- **Let the operator choose the ICU version.** Would allow lining up with a backend's UCA
  ...
  vary per deployment in ways support cannot reason about.
...
- `ICU` alignment is explicitly **best-effort**: ICU4J 77.1 ships a much newer UCA/CLDR
...
identity on a CI backend is therefore already broken today — the change *repairs* existing
...
- The alignment contract for key identity is strictly stronger than for ordering/equality
  (it adds the uniqueness and recovery point-read requirements) and is documented as such.
...
construction* reject `ICU` at startup; this record still governs every configurable backend
...
- The operator owns the consequences of misalignment; the documentation states this
...
Two integration-test suites exercise `ICU` at `PRIMARY` strength inside the existing CI
jobs — `DistributedStorageCollationIntegrationTestBase` (storage layer) and
`ConsensusCommitCollationIntegrationTestBase` (key identity) — with the backend collation
applied through test-only `AdminTestUtils` hooks:
...
- **Default-mode fidelity:** the CI jobs' default collations are pinned to exact `BINARY`
  ...
  `Japanese_BIN2`, AlloyDB posix — `ci/tests-config.yaml`) so the rest of the test matrix
  exercises the default mode against genuinely byte-ordered backends.

### Consequences

- The aligned-backend contract of ADR-5 is exercised end-to-end on MySQL 8.x, MariaDB
  10.10+, PostgreSQL/AlloyDB, and SQL Server; object storage runs neither suite — it
...
"match" is not a binary property. But for a subset of storages no probe is needed — the
storage *kind* alone proves `ICU` can never match:
...
- **Implement object-storage `ICU` key identity instead of rejecting.** Deferred: the
  ...
  of the on-disk format — a persisted collation fingerprint, a hard (not warning-level)
  ICU4J version check, and a re-keying migration on every collation or ICU4J change —
  which is disproportionate to current demand for `ICU` on object storage.
