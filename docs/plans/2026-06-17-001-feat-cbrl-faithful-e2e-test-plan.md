...
**Verification:** All three layers report clean on a correct restore; each layer is independently invocable (so U9 can target each). Copy-consistency (C3/R4) is **not** a layer here: a non-closed copy makes the restore fail loud (`IntegrityChecker` throws during repair), and that mechanism's teeth are unit-tested by §6.2 P3 — re-checking it at the E2E level detects nothing a successful restore doesn't already imply.
...
**Not a layer — copy-consistency (C3/R4).** An earlier draft had a "chain-integrity" layer asserting the replay raises no `CbrlReplayException`. It was dropped: the condition it checks (a dangling `prev_tx_id`) is exactly what makes the restore *throw*, so on a successful restore it detects nothing, and on a broken copy the restore already fails. Its only real value — proving the integrity check *fires* — is the unit test §6.2 P3 (drop a mid-chain op ⇒ `IntegrityChecker` throws), which belongs at the core level, not the E2E.
