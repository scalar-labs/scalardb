...
public class ConsensusCommitWithDynamoIntegrationTest {
  ...
  @Before
  public void setUp() {
    DistributedStorage storage = spy(new Dynamo(client));
  ...
  private static void truncateTable(String namespace, String table) {
    ScanRequest request =
        ScanRequest.builder().tableName(namespace + "." + table).consistentRead(true).build();
    client
        .scan(request)
        .items()
        .forEach(
            i -> {
              ...
              DeleteItemRequest delRequest =
                  DeleteItemRequest.builder().tableName(namespace + "." + table).key(key).build();
  ...
  public static void tearDownAfterClass() {
    for (String table : Arrays.asList(TABLE_1, TABLE_2)) {
      client.deleteTable(DeleteTableRequest.builder().tableName(NAMESPACE + "." + table).build());
    }

    client.deleteTable(
        DeleteTableRequest.builder()
            .tableName(Coordinator.NAMESPACE + "." + Coordinator.TABLE)
            .build());
  ...
  private static void insertMetadata() {
    final String metadataTable = METADATA_KEYSPACE + "." + METADATA_TABLE;

    // For the coordinator table
    Map<String, AttributeValue> values = new HashMap<>();
    values.put(
        "table",
        AttributeValue.builder().s(Coordinator.NAMESPACE + "." + Coordinator.TABLE).build());
    values.put(
        "partitionKey",
    ...
    for (String table : Arrays.asList(TABLE_1, TABLE_2)) {
      values = new HashMap<>();
      values.put("table", AttributeValue.builder().s(NAMESPACE + "." + table).build());
      values.put(
          "partitionKey",
