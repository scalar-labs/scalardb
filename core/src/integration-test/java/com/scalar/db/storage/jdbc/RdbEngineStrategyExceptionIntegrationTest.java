...
public class RdbEngineStrategyExceptionIntegrationTest {
  ...
  @Test
  @DisabledIf("isCreateIndexIfNotExistsSyntaxSupported")
  public void isDuplicateIndexError_WhenUnrelatedError_ShouldReturnFalse() {
    // Runs on ALL backends — no assumeTrue guard (D-02)
    // On PostgreSQL/SQLite/DB2 the method always returns false; this verifies that contract
    // Provoke an undefined table error
    SQLException caught = null;
    try {
      executeSql(
          "SELECT 1 FROM "
    ...
    assertThat((Object) caught).isNotNull();
    assertThat(rdbEngine.isDuplicateIndexError(caught)).isFalse();
  }

  private boolean isCreateIndexIfNotExistsSyntaxSupported() {
    return JdbcTestUtils.isDb2(rdbEngine) || JdbcTestUtils.isSqlite(rdbEngine) || JdbcTestUtils.isPostgresql(rdbEngine);
  }
  ...
  public void isConflict_WhenDeadlock_ShouldReturnTrue() throws Exception {
    ...
    try {
      ...
      if (isSqlite) {
        String sqliteNoBusyUrl =
           jdbcUrl.replaceAll("busy_timeout=\\d+", "busy_timeout=0");
