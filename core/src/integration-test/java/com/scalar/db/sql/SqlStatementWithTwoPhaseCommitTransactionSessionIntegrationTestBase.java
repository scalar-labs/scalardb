...
public abstract class SqlStatementWithTwoPhaseCommitTransactionSessionIntegrationTestBase {

  private static final String TEST_NAME = "sql_stmt_2pc";
  ...
  private void createTables(DatabaseConfig config) {
    ...
    try (SqlStatementSessionFactory factory =
        SqlStatementSessionFactory.builder().withProperties(config.getProperties()).build()) {
      SqlStatementSession session = factory.getTwoPhaseCommitTransactionSession();
      session.execute(
  ...
  @Test
  public void executeAndCommit_WithMultipleSubTransactions_ShouldExecuteProperly() {
    // Arrange
    populateRecords();
    SqlStatementSession session1 = sqlStatementSessionFactory.getTwoPhaseCommitTransactionSession();
    SqlStatementSession session2 = sqlStatementSessionFactory.getTwoPhaseCommitTransactionSession();
    ...
    // Act
    session1.begin();
    session2.join(session1.getTransactionId());
    ...
    assertThat(record1).isPresent();
    assertThat(record1.get().getInt(BALANCE)).isEqualTo(expected);
    assertThat(record2).isNotPresent();
  }

  @Test
  public void executeAndRollback_WithMultipleSubTransactions_ShouldExecuteProperly() {
    // Arrange
    populateRecords();
    SqlStatementSession session1 = sqlStatementSessionFactory.getTwoPhaseCommitTransactionSession();
    SqlStatementSession session2 = sqlStatementSessionFactory.getTwoPhaseCommitTransactionSession();
    ...
    session1.execute(
        StatementBuilder.update(namespace, TABLE)
            .set(Assignment.column(BALANCE).value(Value.ofInt(record.get().getInt(BALANCE) + 100)))
            .where(Predicate.column(ACCOUNT_ID).isEqualTo(Value.ofInt(0)))
            .and(Predicate.column(ACCOUNT_TYPE).isEqualTo(Value.ofInt(0)))
            .build());
