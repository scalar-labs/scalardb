...
public class ConsensusCommitManager extends AbstractDistributedTransactionManager {
  ...
  public void finishTransaction(String txId) throws TransactionException {
    ...
    // rolled back, lazy-recovered, or originated from a pre-feature binary) and so carries no
    // write set.
    ...
    // Walk every EntryGroup and run per-record recovery. The decoder rebuilds a Get for each
    // entry; the recovery executor decides rollforward vs. rollback based on the parent state.
    // Records that are already terminal or belong to a different transaction are filtered out
    // up front. The composer-level NoMutationException absorption still acts as the safety net
    // for races against concurrent recovery.
    ...
    for (EntryGroup entryGroup : writeSet.getEntryGroupsList()) {
      ...
      for (Entry entry : entryGroup.getEntriesList()) {
        ...
        if (!resultOpt.isPresent()) {
          // Record is gone (concurrently deleted, or it was a Delete entry whose target had not
          // yet been written). Nothing to recover.
          continue;
        }
        ...
        try {
          recoveryExecutor.executeSynchronously(get, txResult, Optional.of(state));
