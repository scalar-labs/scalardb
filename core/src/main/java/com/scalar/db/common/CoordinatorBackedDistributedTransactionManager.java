 ...
 * participant, and returns a {@link CoordinatorBackedDistributedTransaction}. The full two-phase
 ...
 */
public class CoordinatorBackedDistributedTransactionManager
  ...
  public CoordinatorBackedDistributedTransactionManager(
  ...
  private DistributedTransaction beginInternal(
    ...
    return new CoordinatorBackedDistributedTransaction(
        coordinator, participant, canonicalId, /* joined= */ false);
  }
  ...
  public DistributedTransaction join(String txId) throws TransactionException {
    ...
    return new CoordinatorBackedDistributedTransaction(
        coordinator, participant, txId, /* joined= */ true);
  }
  ...
  private void checkParticipantConfigured() {
    if (participant == null) {
      throw new UnsupportedOperationException(
          "This operation is not supported by this Coordinator-backed transaction manager because no"
  ...
  public TransactionState getState(String txId) {
    throw new UnsupportedOperationException(
        "getState is not supported by the Coordinator-backed transaction manager");
  }

  @Override
  public boolean finishTransaction(String txId) {
    throw new UnsupportedOperationException(
        "finishTransaction is not supported by the Coordinator-backed transaction manager");
  }
  ...
  public boolean recoverRecord(
    ...
    throw new UnsupportedOperationException(
        "recoverRecord is not supported by the Coordinator-backed transaction manager");
  }

  @Override
  public TransactionState rollback(String txId) {
    throw new UnsupportedOperationException(
        "rollback/abort by transaction ID is not supported by the Coordinator-backed transaction"
            + " manager");
  }
