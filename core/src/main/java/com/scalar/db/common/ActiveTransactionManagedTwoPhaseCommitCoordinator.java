 ...
 * {@code releaseTransactionContext} safe against an in-flight {@code commit}/{@code rollback}.
 * Second, the sweep and {@link #registerParticipant} shake hands on the tracked entry's monitor: a
 * registration pushes the expiration time out under the monitor <em>before</em> delegating to the
 * wrapped coordinator, and the sweep re-checks under the same monitor that the expiration time has
 * not moved before releasing and removing. A reap therefore never overlaps a registration that is
 * about to succeed — either the registration extends the deadline first and the sweep backs off, or
 * the reap completes first and the delegated registration is rejected by the wrapped coordinator,
 * whose context is already released. Probing itself is I/O and runs outside the monitor, so a slow
