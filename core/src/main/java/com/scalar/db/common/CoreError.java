...
public enum CoreError implements ScalarDbError {
  ...
  OPERATION_CHECK_ERROR_INDEX_ONLY_SINGLE_COLUMN_INDEX_SUPPORTED(
      ...
      "",
      "Use a single-column index. Create separate indexes for each column if needed."),
  OPERATION_CHECK_ERROR_INDEX_NON_INDEXED_COLUMN_SPECIFIED(
      ...
      "",
      "Create an index on the column or use a different column that has an index."),
  OPERATION_CHECK_ERROR_INDEX_INDEX_KEY_NOT_PROPERLY_SPECIFIED(
      ...
      "",
      "Verify the index key matches the indexed column name and data type in the table metadata."),
  OPERATION_CHECK_ERROR_INDEX_CLUSTERING_KEY_SPECIFIED(
      Category.USER_ERROR,
      "0003",
      "Clustering keys cannot be specified when using an index. Operation: %s",
      "",
      "Remove clustering keys from your operation when using an index."),
  ...
  OPERATION_CHECK_ERROR_CROSS_PARTITION_SCAN_ORDERING(
      ...
      "",
      "Enable cross-partition scan ordering by setting 'scalar.db.cross_partition_scan.ordering.enabled' to 'true' in your configuration."),
  OPERATION_CHECK_ERROR_CROSS_PARTITION_SCAN_FILTERING(
      Category.USER_ERROR,
      "0008",
      "Cross-partition scan filtering is not enabled. Operation: %s",
      "",
      "Enable cross-partition scan filtering by setting 'scalar.db.cross_partition_scan.filtering.enabled' to 'true' in your configuration."),
  OPERATION_CHECK_ERROR_PROJECTION(
      Category.USER_ERROR,
      "0009",
      "The specified projection is not found. Projection: %s, Operation: %s; Table metadata: %s",
      "",
      "Verify that the projection column exists in the table metadata and correct the column name."),
  ...
  OPERATION_CHECK_ERROR_START_CLUSTERING_KEY(
      ...
      "",
      "Verify that the start clustering-key columns match the table's clustering-key definition and are specified in the correct order."),
  OPERATION_CHECK_ERROR_END_CLUSTERING_KEY(
      Category.USER_ERROR,
      "0012",
      "The end clustering key is not properly specified. Operation: %s; Table metadata: %s",
      "",
      "Verify that the end clustering-key columns match the table's clustering-key definition and are specified in the correct order."),
  OPERATION_CHECK_ERROR_ORDERING_NOT_PROPERLY_SPECIFIED(
      Category.USER_ERROR,
      "0013",
      "Orderings are not properly specified. Operation: %s; Table metadata: %s",
      "",
      "Ensure the ordering specifies clustering-key columns in the correct order and that ordering directions (ASC/DESC) are valid."),
  OPERATION_CHECK_ERROR_ORDERING_COLUMN_NOT_FOUND(
      ...
      "",
      "Verify that the ordering column exists in the table metadata and is a clustering-key column."),
  ...
  OPERATION_CHECK_ERROR_PARTITION_KEY(
      ...
      "",
      "Ensure all partition key columns are specified with the correct names and data types in the proper order."),
  OPERATION_CHECK_ERROR_CLUSTERING_KEY(
      Category.USER_ERROR,
      "0021",
      "The clustering key is not properly specified. Operation: %s; Table metadata: %s",
      "",
      "Verify that the clustering-key columns match the table's definition and are specified in the correct order."),
  AUTH_NOT_ENABLED(
      Category.USER_ERROR,
      "0022",
      ...
      "",
      "Enable authentication and authorization in your ScalarDB Enterprise configuration, or remove authentication-related operations from your code."),
  ...
  LIKE_CHECK_ERROR_LIKE_PATTERN_MUST_NOT_INCLUDE_ONLY_ESCAPE_CHARACTER(
      ...
      "",
      "Include pattern characters in addition to the escape character."),
  ...
  GET_BUILD_ERROR_OPERATION_NOT_SUPPORTED_WHEN_GETTING_RECORDS_OF_DATABASE_WITHOUT_USING_INDEX(
      ...
      "",
      "Remove the unsupported operation or use an index for the Get operation."),
  GET_BUILD_ERROR_OPERATION_NOT_SUPPORTED_WHEN_GETTING_RECORDS_OF_DATABASE_USING_INDEX(
      ...
      "",
      "Remove the unsupported operation or do not use an index for the Get operation."),
  SCAN_BUILD_ERROR_OPERATION_NOT_SUPPORTED_WHEN_SCANNING_ALL_RECORDS_OF_DATABASE_OR_SCANNING_RECORDS_OF_DATABASE_USING_INDEX(
      ...
      "",
      "Remove the unsupported operation or use a partition key for the Scan operation."),
  SCAN_BUILD_ERROR_OPERATION_SUPPORTED_ONLY_WHEN_SCANNING_RECORDS_OF_DATABASE_USING_INDEX(
      ...
      "",
      "Use an index for the Scan operation."),
  SCAN_BUILD_ERROR_OPERATION_NOT_SUPPORTED_WHEN_SCANNING_RECORDS_OF_DATABASE_USING_INDEX(
      ...
      "",
      "Remove the unsupported operation or do not use an index for the Scan operation."),
  ...
  TABLE_METADATA_BUILD_ERROR_PARTITION_KEY_COLUMN_DEFINITION_NOT_SPECIFIED(
      ...
      "",
      "Add the column definition for the partition key column."),
  ...
  OPERATION_DOES_NOT_HAVE_TARGET_NAMESPACE_OR_TABLE_NAME(
      ...
      "",
      "Specify both the namespace and table name in the operation."),
  CONFIG_UTILS_INVALID_NUMBER_FORMAT(
      Category.USER_ERROR,
      "0059",
      "The specified value of the property '%s' is not a number. Value: %s",
      "",
      "Provide a valid numeric value for the configuration property."),
  ...
  GET_OPERATION_USED_FOR_NON_EXACT_MATCH_SELECTION(
      ...
      "",
      "Use 'scan()' instead of 'get()' for range queries or partial key matching."),
  ...
  INVALID_CONTACT_PORT(
      ...
      "",
      "Set 'scalar.db.contact_port' to a valid port number (0-65535)."),
  COSMOS_CLUSTERING_KEY_BLOB_TYPE_NOT_SUPPORTED(
      Category.USER_ERROR,
      "0073",
      "The BLOB type is not supported for clustering keys in Cosmos DB. Column: %s",
      "",
      "Use a different data type for the clustering-key column."),
  COSMOS_IMPORT_NOT_SUPPORTED(
      Category.USER_ERROR,
      "0074",
      "Import-related functionality is not supported in Cosmos DB",
      "",
      "Use schema creation and data loading instead of the import functionality for Cosmos DB."),
  INVALID_CONTACT_POINTS(
      Category.USER_ERROR,
      "0075",
      "The property 'scalar.db.contact_points' must not be empty",
      "",
      "Provide at least one contact point in the 'scalar.db.contact_points' configuration property."),
  ...
  DYNAMO_INDEX_COLUMN_CANNOT_BE_SET_TO_EMPTY(
      ...
      "",
      "Provide a non-null, non-empty value for indexed Text or Blob columns in DynamoDB."),
  DYNAMO_CONDITION_OPERATION_NOT_SUPPORTED_FOR_BOOLEAN_TYPE(
      ...
      "",
      "Use only EQ, NE, IS_NULL, or IS_NOT_NULL operators for BOOLEAN columns in DynamoDB conditions."),
  ...
  JDBC_TRANSACTION_GETTING_TRANSACTION_STATE_NOT_SUPPORTED(
      ...
      "",
      "Remove the call to get a transaction state when using JDBC transactions."),
  JDBC_TRANSACTION_ROLLING_BACK_TRANSACTION_NOT_SUPPORTED(
      Category.USER_ERROR,
      "0093",
      "Rolling back a transaction is not supported in JDBC transactions",
      "",
      "Remove the rollback call. JDBC transactions in ScalarDB do not support manual rollback."),
