...
public interface BranchTransaction extends TransactionCrudOperable {
   ...
   * owning {@link GlobalTransaction} is committed or rolled back. Every scanner obtained from this
   * branch must be closed first. Issuing CRUD on this branch once {@code end()} has been called is
   * not allowed and is rejected with {@link IllegalStateException}, as is calling {@code end()}
   * again.
   *
   * @throws IllegalStateException if the branch has already been ended, or a scanner obtained from
   *     this branch has not been closed
