 ...
 * <h3>Encoding</h3>
 *
 * <p>{@code Struct} member values are encoded onto the target byte[] in the order in which they are
 ...
 *
 * <p>{@code Struct} treats the right-most nullable field members as special. Rather than writing
 * null values to the output buffer, {@code Struct} omits those records all together. When reading
 * back a value, it will look for the scenario where the end of the buffer has been reached but
 * there are still nullable fields remaining in the {@code Struct} definition. When this happens, it
 * will produce null entries for the remaining values. For example:
 *
 * <pre>
 ...
 * <h3>Sort Order</h3>
 *
 * <p>{@code Struct} instances sort according to the composite order of their fields, that is,
...
public class Struct implements DataType<Object[]> {
  ...
  /**
   * Create a new {@code Struct} instance defined as the sequence of {@code HDataType}s in {@code
   * memberTypes}.
   *
   * <p>A {@code Struct} is {@code orderPreserving} when all of its fields are {@code
   * orderPreserving}. A {@code Struct} is {@code skippable} when all of its fields are {@code
   * skippable}.
   */
  @SuppressWarnings("rawtypes")
