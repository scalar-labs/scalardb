 ...
 *
 * <p>{@code ByteRange} maintains an underlying byte[] and a viewport into that byte[] as a range of
 ...
 * methods.
 *
 * <p>This interface differs from ByteBuffer:
 *
 * <ul>
 *   <li>On-heap bytes only
 *   <li>Raw {@code byte} access only; does not encode other primitives.
 *   <li>Implements {@code equals(Object)}, {@code #hashCode()}, and {@code #compareTo(ByteRange)}
 *       so that it can be used in standard java Collections. Comparison operations are
 *       lexicographic, which is native to HBase.
 *   <li>Allows the addition of simple core methods like the deep and shallow copy methods.
 *   <li>Can be reused in tight loops like a major compaction which can save significant amounts of
 *       garbage. (Without reuse, we throw off garbage like <a
 *       href="http://www.youtube.com/watch?v=lkmBH-MjZF4">this thing</a>.)
 * </ul>
 *
 * <p>Mutable, and always evaluates {@code #equals(Object)}, {@code #hashCode()}, and {@code
 * #compareTo(ByteRange)} based on the current contents.
 *
 * <p>Can contain convenience methods for comparing, printing, cloning, spawning new arrays, copying
 * to other arrays, etc. Please place non-core methods into {@link ByteRangeUtils}.
 */
...
public interface ByteRange extends Comparable<ByteRange> {
  ...
  /**
   * Update the beginning of this range. {@code offset + length} may not be greater than {@code
   * bytes.length}.
   *
  ...
  /**
   * Update the length of this range. {@code offset + length} should not be greater than {@code
   * bytes.length}.
   *
   ...
   * @param destinationOffset First index in the destination array.
   */
  void deepCopySubRangeTo(
      int innerOffset, int copyLength, byte[] destination, int destinationOffset);
