...
class AvroSchemaGenerator {
  ...
  private final ConcurrentHashMap<ObjectStorageTableMetadata, Schema> schemaCache =
      new ConcurrentHashMap<>();

  Schema getRecordSchema(ObjectStorageTableMetadata metadata) {
    return schemaCache.computeIfAbsent(metadata, this::buildRecordSchema);
  }

  private Schema buildRecordSchema(ObjectStorageTableMetadata metadata) {
