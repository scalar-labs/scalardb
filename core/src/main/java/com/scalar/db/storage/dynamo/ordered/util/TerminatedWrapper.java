...
public class TerminatedWrapper<T> implements DataType<T> {
  ...
  /**
   * Create a terminated version of the {@code wrapped}. {@code term} is converted to a {@code
   * byte[]} using {@link Bytes#toBytes(String)}.
   *
   * @throws IllegalArgumentException when {@code term} is null or empty.
   */
  ...
  /**
   * Return the position at which {@code term} begins within {@code src}, or {@code -1} if {@code
   * term} is not found.
   */
  protected int terminatorPosition(PositionedByteRange src) {
    ...
    int i;
    SKIP:
    for (i = src.getPosition(); i < src.getLength(); i++) {
