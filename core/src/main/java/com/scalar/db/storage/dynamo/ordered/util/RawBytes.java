...
/**
 * An {@code DataType} for interacting with variable-length values encoded using {@link
 * Bytes#putBytes(byte[], int, byte[], int, int)}. Intended to make it easier to transition away
 * from direct use of {@link Bytes}.
 *
...
public class RawBytes implements DataType<byte[]> {
  ...
  @Deprecated public static final RawBytes ASCENDING = new RawBytes(Order.ASCENDING);
  /** @deprecated since 3.0.0 and will be removed in 4.0.0 */
  @Deprecated public static final RawBytes DESCENDING = new RawBytes(Order.DESCENDING);
