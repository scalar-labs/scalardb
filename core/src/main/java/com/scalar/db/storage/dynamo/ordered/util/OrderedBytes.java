...
import static com.scalar.db.storage.dynamo.ordered.util.Order.ASCENDING;
import static com.scalar.db.storage.dynamo.ordered.util.Order.DESCENDING;
 ...
 *
 * <p>Each value is encoded as one or more bytes. The first byte of the encoding, its meaning, and a
 ...
 * <h3>Null Encoding</h3>
 *
 * <p>Each value that is a NULL encodes as a single byte of 0x05. Since every other value encoding
 * begins with a byte greater than 0x05, this forces NULL values to sort first.
 *
 * <h3>Text Encoding</h3>
 *
 * <p>Each text value begins with a single byte of 0x33 and ends with a single byte of 0x00. There
 * are zero or more intervening bytes that encode the text value. The intervening bytes are chosen
 * so that the encoding will sort in the desired collating order. The intervening bytes may not
 * contain a 0x00 character; the only 0x00 byte allowed in a text encoding is the final byte.
 *
 * <p>The text encoding ends in 0x00 in order to ensure that when there are two strings where one is
 * a prefix of the other that the shorter string will sort first.
 *
 * <h3>Binary Encoding</h3>
 *
 * <p>There are two encoding strategies for binary fields, referred to as "BlobVar" and "BlobCopy".
 ...
 * <h4>BlobVar</h4>
 *
 * <p>"BlobVar" encodes the input byte[] in a manner similar to a variable length integer encoding.
 * As with the other {@code OrderedBytes} encodings, the first encoded byte is used to indicate what
 ...
 * <h4>BlobCopy</h4>
 *
 * <p>"BlobCopy" is a simple byte-for-byte copy of the input data. It uses 0x38 as the header byte,
 * and is terminated by 0x00 in the DESCENDING case. This alternative encoding is faster and more
 ...
 *
 * <p>Numeric values must be coded so as to sort in numeric order. We assume that numeric values can
 * be both integer and floating point values. Clients must be careful to use inspection methods for
 * encoded values (such as {@link #isNumericInfinite(PositionedByteRange)} and {@link
 * #isNumericNaN(PositionedByteRange)} to protect against decoding values into object which do not
 * support these numeric concepts (such as {@link Long} and {@link BigDecimal}).
 *
 * <p>Simplest cases first: If the numeric value is a NaN, then the encoding is a single byte of
 * 0x25. This causes NaN values to sort after every other numeric value.
 *
 * <p>If the numeric value is a negative infinity then the encoding is a single byte of 0x07. Since
 ...
 *
 * <p>If the numeric value is a positive infinity then the encoding is a single byte of 0x23. Every
 ...
 *
 * <p>If the numeric value is exactly zero then it is encoded as a single byte of 0x15. Finite
 * negative values will have initial bytes of 0x08 through 0x14 and finite positive values will have
 * initial bytes of 0x16 through 0x22.
 *
 * <p>For all numeric values, we compute a mantissa M and an exponent E. The mantissa is a base-100
 * representation of the value. The exponent E determines where to put the decimal point.
 *
 * <p>Each centimal digit of the mantissa is stored in a byte. If the value of the centimal digit is
 * X (hence X&ge;0 and X&le;99) then the byte value will be 2*X+1 for every byte of the mantissa,
 ...
 *
 * <p>If we assume all digits of the mantissa occur to the right of the decimal point, then the
 * exponent E is the power of one hundred by which one must multiply the mantissa to recover the
 * original value.
 *
 * <p>Values are classified as large, medium, or small according to the value of E. If E is 11 or
 * more, the value is large. For E between 0 and 10, the value is medium. For E less than zero, the
 * value is small.
 *
 * <p>Large positive values are encoded as a single byte 0x22 followed by E as a varint and then M.
 ...
 *
 * <p>Small negative values are encoded as a single byte 0x14 followed by -E as a varint and then
 * the ones-complement of M. Medium negative values are encoded as a byte 0x13-E followed by the
 ...
 *
 * <p>All 4-byte integers are serialized to a 5-byte, fixed-width, sortable byte format. All 8-byte
 ...
 *
 * <p>32-bit and 64-bit floating point numbers are encoded to a 5-byte and 9-byte encoding format,
 ...
 *
 * <p>Floating point numbers are encoded as specified in IEEE 754. A 32-bit single precision float
 * consists of a sign bit, 8-bit unsigned exponent encoded in offset-127 notation, and a 23-bit
 * significand. The format is described further in the <a
 * href="http://en.wikipedia.org/wiki/Single_precision">Single Precision Floating Point Wikipedia
 * page</a>
 *
 * <p>The value of a normal float is -1 <sup>sign bit</sup> &times; 2<sup>exponent - 127</sup>
 * &times; 1.significand
 *
 * <p>The IEE754 floating point format already preserves sort ordering for positive floating point
 ...
 *
 * <p>Thus, we need only ensure that negative numbers sort in the the exact opposite order as
 * positive numbers (so that say, negative infinity is less than negative 1), and that all negative
 * numbers compare less than any positive number. To accomplish this, we invert the sign bit of all
 * floating point numbers, and we also invert the exponent and significand bits if the floating
 * point number was negative.
 *
 * <p>More specifically, we first store the floating point bits into a 32-bit int {@code j} using
 ...
 * </pre>
 *
 * <p>which inverts the sign bit and XOR's all other bits with the sign bit itself. Comparing the
 * raw bytes of {@code j} in most significant byte order is equivalent to performing a single
 ...
 *
 * <p>The resulting integer is then converted into a byte array by serializing the integer one byte
 ...
 *
 * <p>{@code OrderedBytes} encodings are heavily influenced by the <a
 * href="http://sqlite.org/src4/doc/trunk/www/key_encoding.wiki">SQLite4 Key Encoding</a>. Slight
...
public class OrderedBytes {
  ...
  private static BigDecimal decodeSignificand(PositionedByteRange src, int e, boolean comp) {
    ...
    for (int i = 0; ; i++) {
      if (i > remaining) {
        ...
        throw new IllegalArgumentException(
            "Read exceeds range before termination byte found. offset: "
                + offset
                + " position: "
                + (start + i));
      }
      ...
      m =
          m.add( // m +=
              new BigDecimal(BigInteger.ONE, e * -2)
                  .multiply( // 100 ^ p * [decoded digit]
   ...
   *
   * <p>A floating point value is encoded as an integer exponent {@code E} and a mantissa {@code M}.
   ...
   *
   * <p>For this routine, {@code E} will always be zero or negative, since the original value is
   * less than one. The encoding written by this routine is the ones-complement of the varint of the
   ...
   * possible without making {@code M} greater than or equal to 1.0.
   *
   * <p>Each centimal digit of the mantissa is stored in a byte. If the value of the centimal digit
   * is {@code X} (hence {@code X>=0} and {@code X<=99}) then the byte value will be {@code 2*X+1}
   * for every byte of the mantissa, except for the last byte which will be {@code 2*X+0}. The
   * mantissa must be the minimum number of bytes necessary to represent the value; trailing {@code
   * X==0} digits are omitted. This means that the mantissa will never contain a byte with the value
   * {@code 0x00}.
   *
   * <p>If {@code E > 10}, then this routine writes of {@code E} as a varint followed by the
   * mantissa as described above. Otherwise, if {@code E <= 10}, this routine only writes the
   * mantissa and leaves the {@code E} value to be encoded as part of the opening byte of the field
   ...
   * {@code double}, this method performs an implicit narrowing conversion as described in {@link
   ...
   * {@link BigDecimal}; in the event the encoded value is larger than can be represented in a
   * {@code long}, this method performs an implicit narrowing conversion as described in {@link
   * BigDecimal#doubleValue()}.
   *
   * @throws NullPointerException when the encoded value is {@code NULL}.
   * @throws IllegalArgumentException when the encoded value is not a Numeric.
  ...
  /**
   * Encode a String value. String encoding is 0x00-terminated and so it does not support {@code
   * \u0000} codepoints in the value.
   *
   ...
   *
   * <p>This format encodes a byte[] value such that no limitations on the input value are imposed.
   * The first byte encodes the encoding scheme that follows, {@link #BLOB_VAR}. Each encoded byte
   ...
   * terminated so as to preserve proper sorting of {@code []} and so it does not support {@code
   * 0x00} in the value.
   *
   * @return the number of bytes written.
   ...
   *
   * <p>This format ensures that all longs sort in their natural order, as they would sort when
   * using signed long comparison.
   *
   * <p>All Longs are serialized to an 8-byte, fixed-width sortable byte format. Serialization is
   ...
   * </pre>
   *
   * <p>This encoding format, and much of this documentation string, is based on Orderly's {@code
   * FixedIntWritableRowKey}.
   *
   * @return the number of bytes written.
   ...
   * Double.NaN
   *
   * <p>Floating point numbers are encoded as specified in IEEE 754. A 64-bit double precision float
   * consists of a sign bit, 11-bit unsigned exponent encoded in offset-1023 notation, and a 52-bit
   * significand. The format is described further in the <a
   * href="http://en.wikipedia.org/wiki/Double_precision">Double Precision Floating Point Wikipedia
   * page</a>
   *
   * <p>The value of a normal float is -1 <sup>sign bit</sup> &times; 2<sup>exponent - 1023</sup>
   * &times; 1.significand
   *
   * <p>The IEE754 floating point format already preserves sort ordering for positive floating point
   * numbers when the raw bytes are compared in most significant byte order. This is discussed
   * further at <a href= "http://www.cygnus-software.com/papers/comparingfloats/comparingfloats.htm"
   * > http://www.cygnus-software.com/papers/comparingfloats/comparingfloats. htm</a>
   *
   * <p>Thus, we need only ensure that negative numbers sort in the the exact opposite order as
   ...
   *
   * <p>More specifically, we first store the floating point bits into a 64-bit long {@code l} using
   ...
   *
   * <p>which inverts the sign bit and XOR's all other bits with the sign bit itself. Comparing the
   * raw bytes of {@code l} in most significant byte order is equivalent to performing a double
   * precision floating point comparison on the underlying bits (ignoring NaN comparisons, as NaNs
   * don't compare equal to anything when performing floating point comparisons).
   *
   * <p>The resulting long integer is then converted into a byte array by serializing the long one
   * byte at a time in most significant byte order. The serialized integer is prefixed by a single
   * header byte. All serialized values are 9 bytes in length.
   *
   * <p>This encoding format, and much of this highly detailed documentation string, is based on
  ...
  /**
   * Return true when the next encoded value in {@code src} uses Numeric encoding and is {@code
