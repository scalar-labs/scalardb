...
public class ConcatenationVisitorTest {
  ...
  public void build_AllTypesGiven_ShouldBuildString() {
    ...
    // Assert
    String[] values =
        actual.split(String.valueOf(ObjectStorageUtils.CONCATENATED_KEY_DELIMITER), -1);
    ...
    assertThat(values[7])
        .isEqualTo(String.valueOf(TimeRelatedColumnEncodingUtils.encode(ANY_DATE_COLUMN)));
    assertThat(values[8])
        .isEqualTo(String.valueOf(TimeRelatedColumnEncodingUtils.encode(ANY_TIME_COLUMN)));
    assertThat(values[9])
        .isEqualTo(String.valueOf(TimeRelatedColumnEncodingUtils.encode(ANY_TIMESTAMP_COLUMN)));
    assertThat(values[10])
        .isEqualTo(String.valueOf(TimeRelatedColumnEncodingUtils.encode(ANY_TIMESTAMPTZ_COLUMN)));
  }
