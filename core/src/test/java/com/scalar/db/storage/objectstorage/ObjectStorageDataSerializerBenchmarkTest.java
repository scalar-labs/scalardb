...
public class ObjectStorageDataSerializerBenchmarkTest {
  ...
  static Stream<Arguments> allSerializerProvider() {
    Stream<Arguments> all =
        Stream.of(
            ...
            Arguments.of(
                "Avro+Deflate", CodecFactory.deflateCodec(CodecFactory.DEFAULT_DEFLATE_LEVEL)),
            Arguments.of("Avro+Snappy", CodecFactory.snappyCodec()));
  ...
  void benchmark(String name, CodecFactory codecFactory) {
    runBenchmark(name, partition, RECORD_COUNT, codecFactory);
  }

  @ParameterizedTest(name = "{0} [small]")
  @MethodSource("allSerializerProvider")
  void benchmarkSmall(String name, CodecFactory codecFactory) {
    runBenchmark(name + " [small]", smallPartition, SMALL_RECORD_COUNT, codecFactory);
  }
  ...
  private void runBenchmark(
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; i++) {
      byte[] data = AvroSerializer.serialize(targetPartition, osMetadata, codecFactory);
      AvroSerializer.deserialize(data, osMetadata);
    }
    ...
    for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
      ...
      byte[] serialized = AvroSerializer.serialize(targetPartition, osMetadata, codecFactory);
      ...
      ObjectStoragePartition deserialized = AvroSerializer.deserialize(serialized, osMetadata);
