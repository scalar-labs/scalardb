...
public class ObjectStorageDataSerializerBenchmarkTest {
  ...
  void benchmark(String name, ObjectStorageDataSerializer serializer) {
    runBenchmark(name, partition, RECORD_COUNT, serializer);
  }

  @ParameterizedTest(name = "{0} [small]")
  @MethodSource("allSerializerProvider")
  void benchmarkSmall(String name, ObjectStorageDataSerializer serializer) {
    runBenchmark(name + " [small]", smallPartition, SMALL_RECORD_COUNT, serializer);
  }
  ...
  private void runBenchmark(
    ...
    // Warmup with target data
    System.gc();
    for (int i = 0; i < WARMUP_ITERATIONS; i++) {
      byte[] data = serializer.serialize(targetPartition);
    ...
    for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
      ...
      byte[] serialized = serializer.serialize(targetPartition);
