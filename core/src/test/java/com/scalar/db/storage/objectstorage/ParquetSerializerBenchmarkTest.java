...
public class ParquetSerializerBenchmarkTest {
  ...
  void benchmark(String name, CompressionCodecName codec) {
    runBenchmark(name, partition, RECORD_COUNT, codec);
  }

  @ParameterizedTest(name = "{0} [small]")
  @MethodSource("allCompressionProvider")
  void benchmarkSmall(String name, CompressionCodecName codec) {
    runBenchmark(name + " [small]", smallPartition, SMALL_RECORD_COUNT, codec);
  }
  ...
  private void runBenchmark(
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; i++) {
      byte[] data = ParquetSerializer.serialize(targetPartition, metadata, codec);
    ...
    for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
      ...
      byte[] serialized = ParquetSerializer.serialize(targetPartition, metadata, codec);
